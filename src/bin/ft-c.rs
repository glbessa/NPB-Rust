...
const COMPILETIME: &str = "27 Oct 2020";
const COMPILERVERSION: &str = "rustc"; //1.48.0-nightly (a4a3b8e3e 2020-09-20)
const CS1: &str = "-";
const CS2: &str = "-";
const CS3: &str = "-";
const CS4: &str = "-";
...
fn main() {
    ...
    println!("Computando indexmap\n");
    compute_indexmap(&mut twiddle);

    println!("Computando condições iniciais");
    compute_initial_conditions(u1.get_mut());
    // compute_initial_conditions(&mut u1);
    // println!("Pós condições iniciais");
    println!("u1[0] = {:?}\n", u1.borrow()[0]);
    println!("u[0] = {:?}\n", u[0]);

    //printa todo u1
    // let mut qntZeros = 0;
    // let mut indexZeros: Vec<usize> = Vec::new();
    // for i in 0..u1.len() {
    //     if u1[i].real != 0.0 || u1[i].imag != 0.0 {
    //         // println!("u1[{}] = {:?}", i, u1[i]);
    //     } else {
    //         qntZeros += 1;
    //         indexZeros.push(i);
    //     }
    // }
    // println!("qntZeros = {}", qntZeros);

    // //printa os indices dos zeros
    // println!("indexZeros[{}] = {}", 0, indexZeros[0]);
    // for i in 1..indexZeros.len() {
    //     if indexZeros[i] != indexZeros[i - 1] + 1 {
    //         println!("indexZeros[{}] = {}", i, indexZeros[i]);
    //     }
    // }
    // println!("indexZeros[{}] = {}", indexZeros.len()-1, indexZeros[indexZeros.len()-1]);
    // println!("qnt índices Zero = {}", indexZeros.len());
    // println!("NX NY NZ = {} {} {}", NX, NY, NZ);

    println!("Computando fft_init\n");
    fft_init(MAXDIM, &mut u);
    // println!("Pós fft_init\n");
    // // println!("u0[0] = {:?}\n", u0.borrow()[0]);
    
    // println!("Computando fft\n");
    fft(1, &u1, &u0, &u);
    println!("u0[0] = {:?}\n", u0.borrow()[0]);
    println!("u[0] = {:?}\n", u[0]);
    // println!("Pós fft\n");
    ...
    println!("Computando FOR\n");
    for iter in 1..=niter {
        evolve(u0.get_mut(), u1.get_mut(), &mut twiddle);
        //#####################################################
        //NÃO ESQUECER DESTA BOSTA
        fft(-1, &u1, &u1, &u);
        //#####################################################
...
// fn fft(dir: i32, tx1: &mut Vec<Dcomplex>, tx2: &mut Vec<Dcomplex>, u: &Vec<Dcomplex>) {
    ...
    if dir == 1 {
        ...
        // println!("fft: cffts1: x1[0] = {:?}", tx1[0]);
        // println!("fft: cffts1: y1[0] = {:?}", y1[0]);
        // println!("fft: cffts1: y2[0] = {:?}", y2[0]);
        // let tx1: Vec<Dcomplex> = (*x1.borrow()).clone();
        cffts2(1, tx1.clone(), &mut tx1, &mut y1, &mut y2, u);

        // println!("fft: cffts2: x1[0] = {:?}", tx1[0]);
        // println!("fft: cffts2: y1[0] = {:?}", y1[0]);
        // println!("fft: cffts2: y2[0] = {:?}", y2[0]);

        // let tx1: Vec<Dcomplex> = (*x1.borrow()).clone();
        cffts3(1, tx1.clone(), &mut tx2, &mut y1, &mut y2, u);

        // println!("fft: cffts3: x1[0] = {:?}", tx1[0]);
        // println!("fft: cffts3: x2[0] = {:?}", tx2[0]);
        // println!("fft: cffts3: y1[0] = {:?}", y1[0]);
        // println!("fft: cffts3: y2[0] = {:?}", y2[0]);
    } else {
    ...
    println!("\n\nfft:tx1[0] = {:?}", tx1[0]);
    println!("fft:tx2[0] = {:?}", tx2[0]);
    println!("fft:x1[0] = {:?}", x1.borrow()[0]);
    println!("fft:x2[0] = {:?}", x2.borrow()[0]);
...
fn cffts1(
    ...
    // let mut soma = 0.0;
    println!("\n################ cffts1 ################");
    // println!("before -> cffts1:y1[0] = {:?}", y1[0]);
    // println!("before -> cffts1:x[0] = {:?}", x[0]);
    for k in 0..D3 {
        for jj in (0..(D2 - FFTBLOCK)).step_by(FFTBLOCK) {
            for j in 0..FFTBLOCK {
                for i in 0..D1 {
                    y1[indxp2(i, j, FFTBLOCK)] = x[indxp3(k, j + jj, i)];
                    // soma += y1[indxp2(i, j, FFTBLOCK)].real;
                    ...
                    //     i,
                    //     j,
                    //     y1[indxp2(i, j, FFTBLOCK)]
                    // );
                    if y1[indxp2(i, j, FFTBLOCK)].real == 0.0 {
                        //panic!("Tá gerando zero....");
                    }
                }
                // panic!("Ué");
                // println!("-----------------");
            ...
            for j in 0..FFTBLOCK {
                for i in 0..D1 {
                    ...
                    //     j + jj,
                    //     i,
                    //     xout[indxp3(k, j + jj, i)]
                    // );
                }
            }
        }
    }
    println!("cffts1:y1[0] = {:?}", y1[0]);
    println!("cffts1:xout[0] = {:?}\n", xout[0]);
    println!("################ FIM ################\n");
}
fn cffts2(
    is: i32,
...
fn cffts3(
    ...
    // let (mut i, mut j, mut k, mut ii): (usize, usize, usize, usize);
...
fn cfftz(
    ...
    let mx = u[0].real;
    // println!("Entrou no cfftz: u[0]: {}", u[0].real);
    if is != 1 && is != -1 || m < 1 || m as f64 > mx {
        println!("CFFTZ: Either U has not been initialized, or else\none of the input parameters is invalid\nis:{:5}\nm:{:5}\nmx:{:5}", is, m, mx);
        println!(
            "{:5} || {:5} || {:5}",
            is != 1 && is != -1,
            m < 1,
...
fn verify(
     ...
     * ---------------------------------------------------------------------
     */
    // let mut csum_ref = Array1::<Dcomplex>::zeros(25 + 1);
